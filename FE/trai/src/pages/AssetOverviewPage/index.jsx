          ...
          {/* Info Cards */}
            <>
              ...
              <div className="info-card-small card2">
                ...
                <div style={{ fontSize: "0.8vw" }}>{assetLoading ||assetError ? " ? " :(assetEmpty ? "자산 정보가 없습니다." : `${assetData.totalPurchase}`)} KRW</div>
                <div className="card-title">총 평가</div>
                <div style={{ fontSize: "0.8vw" }}>{assetLoading ||assetError ? " ? " : (assetEmpty ? "자산 정보가 없습니다." :`${assetData.totalValuation}`)} KRW</div>
              </div>

              <div className="info-card-small card3">
                <div className="card-title">보유 KRW</div>
                <div style={{ fontSize: "0.8vw" }}>{assetLoading||assetError ? " ? " : (assetEmpty ? "자산 정보가 없습니다." :`${assetData.heldKRW}`)}</div>
                <div className="card-title">주문가능금액</div>
                <div style={{ fontSize: "0.8vw" }}>{assetLoading||assetError ? " ? " : (assetEmpty ? "자산 정보가 없습니다." :`${assetData.heldKRW}`)} KRW</div>
              </div>
            ...
            : 
              <div className='card-content'>
                {tradeInitialized === false ? <div> &nbsp;&nbsp;&nbsp;투자가 아직 시작되지 않았습니다. </div> :
                  <>
                    <div>누적 수익률</div>
                    <div style={{ fontSize: "1vw" }}>{`${investmentData.returnRate}`} %</div>
                    <div>총 거래 횟수</div>
